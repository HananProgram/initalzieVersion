            ...
            <div class="bg-white p-6 rounded-xl shadow-md">
                ...
                <form wire:submit.prevent="saveList" class="flex gap-4 items-end">
                    <div class="flex-1 relative">
                        ...
                        <label
                            class="absolute right-3 -top-2.5 px-1 bg-white text-xs text-gray-500 transition-all peer-focus:-top-2.5 peer-focus:text-xs">
                            اسم القائمة
                        </label>
                        @error('newListName')
        ...
        @foreach ($lists as $list)
            <div class="bg-white rounded-xl shadow-md p-6 mb-6">
                ...
                @if (in_array($list->id, $expandedLists))
                    ...
                    @foreach ($list->items as $item)
                        <div class="bg-gray-50 border rounded-lg p-4 mb-4 ml-4 space-y-3">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                @else
                                    <button wire:click="toggleItemExpand({{ $item->id }})"
                                        class="flex items-center gap-2">
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="w-4 h-4 text-gray-500 transform transition-transform duration-200"
                                            :class="{ 'rotate-90': @js(in_array($item->id, $expandedItems)) }" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 5l7 7-7 7" />
                                        </svg>
                            ...
                            @if (in_array($item->id, $expandedItems))
                                ...
                                <div class="flex items-center gap-2 mt-2 pl-8" wire:key="{{ $subKey }}">
                    ...
                    @php
                        $itemKey = 'item-' . $list->id . '-' . ($itemLabel[$list->id] ?? uniqid());
                    @endphp

                    <div class="flex items-center gap-2 ml-4 mt-2" wire:key="{{ $itemKey }}">
                        <input type="text" wire:model="itemLabel.{{ $list->id }}" placeholder="اسم البند"
                            class="w-full rounded-lg border border-gray-300 px-3 py-1.5 text-xs focus:ring-2 focus:ring-primary-500 focus:outline-none shadow-sm">
