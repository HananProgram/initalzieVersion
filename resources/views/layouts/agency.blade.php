    ...
    <nav class="topbar-main" style="background: linear-gradient(90deg, #05997a 0%, #068a8a 100%);">
        ...
        <div class="flex items-center gap-2 sm:gap-4">
            ...
            @elseif($user->isAgencyAdmin())
                ...
                @if(in_array('service_types.view', $userPermissions) || in_array('service_types_view', $userPermissions))
                    <a href="{{ route('agency.service-types') }}" class="topbar-nav-btn {{ request()->routeIs('agency.service-types') ? 'active' : '' }}" title="أنواع الخدمات">
                        <svg class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2a4 4 0 0 1 4-4h2a4 4 0 0 1 4 4v2" /></svg>
            ...
            @else
                ...
                @endif
            @endif
            @if(($user->role && ($user->role->name === 'service_manager' || $user->role->display_name === 'مدير الخدمات')))
                <a href="{{ route('agency.service-types') }}" class="topbar-nav-btn {{ request()->routeIs('agency.service-types') ? 'active' : '' }}" title="إدارة الخدمات">
                    <svg class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M4 7h16M4 12h16M4 17h16"/></svg>
                    <span class="topbar-nav-text">إدارة الخدمات</span>
                </a>
            @endif
